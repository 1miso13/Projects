	...
	//ak nieje prazdny
    {
        ...
        if ((new = malloc(sizeof(struct tBTNode))) != NULL)
		{
			...
			if (parent->Cont > Content) 
                parent->LPtr = new;
			else parent->RPtr = new;
		}
        
    }
    else//ak je prazdny strom
    {
...
**/
    ...
    tStackP stack;
    SInitP(&stack);
    //najprv sa dostaneme do najlavnejsieho 
    Leftmost_Inorder(ptr,&stack);
	while(!SEmptyP(&stack))
    {
        //zo zasobnika vyberieme prvy
        ptr = STopPopP(&stack);
        //spracovanie
        BTWorkOut(ptr);
...
**/
    ...
    while(!SEmptyP(&stackP))
    {
        ...
        {
            SPopP(&stackP);
            if(SSizeP(&stackP)>maxLenght)
            {
                maxLenght=SSizeP(&stackP);
            }
        }
        else//2.
        {
