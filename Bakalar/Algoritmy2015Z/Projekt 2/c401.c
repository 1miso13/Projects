...
**/
    ...
    {
        tBSTNodePtr j;
        if((j= malloc(sizeof(struct tBSTNode)))!=NULL)
        {
            j->Key=K;
            j->BSTNodeCont=Content;
            j->LPtr=NULL;
            j->RPtr=NULL;
            (*RootPtr)=j;
        }
...
**/
    tBSTNodePtr nodePtr=NULL;
    if(*RootPtr!=NULL)
    {
        if(K==(*RootPtr)->Key)//nasli sme ho
        {
            nodePtr=(*RootPtr);
            if((*RootPtr)->RPtr==NULL)
            {
...
**/
    ...
    static int lenght=0;
    if(IsRoot==TRUE && (NodePtr==NULL))//ak bol prazdny
        return -1;
    else
    {
        ...
        {
            lenght++;
            BSTHeight(NodePtr->RPtr, FALSE);
            lenght--;
        }
        if((NodePtr->LPtr==NULL) && (NodePtr->RPtr==NULL) && maxLenght<lenght)
            maxLenght=lenght;
        //lenght--;
    }
    if(IsRoot)
...
**/
    ...
    //najprv skontrolujeme/odstranime lavy
    if(*RootPtr!=NULL)
    {
