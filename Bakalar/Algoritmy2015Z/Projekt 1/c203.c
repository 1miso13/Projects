...
*/
	...
	{
		for(int i=0;i<QUEUE_SIZE;i++) //cyklenie celeho pola
		{
			q->arr[i]='*';			//nastavenie hodnoty kazdeho prvku v poli na '*'
		}
		q->f_index=0;				//index na zaciatok pola
		q->b_index=0;				
	}
...
*/	
	if(queueEmpty (q))			//ak je prazdne pole
	{
		queueError(QERR_FRONT);	//error
	}
	else						
	{
		*c=q->arr[q->f_index];	//z pozicie f_index v poli arr zoberieme znak a vlozime do *c
	}
	//  solved = FALSE;                  /* V pøípadì øe¹ení, sma¾te tento øádek! */
}
...
*/
	if(queueEmpty (q))						//ak je prazdne pole
	{
		queueError(QERR_REMOVE);			//error
	}
	else
	{
		q->f_index=nextIndex(q->f_index); 	//odobratie znaku z pola
	}
	//solved = FALSE;                  /* V pøípadì øe¹ení, sma¾te tento øádek! */
}
...
*/
	...
	{
		queueFront(q,c);		//zoberieme znak a vlozime do c
		queueRemove(q);			//odobereme znak z pola
	}
	// solved = FALSE;                  /* V pøípadì øe¹ení, sma¾te tento øádek! */
}
...
*/
	if(queueFull(q))				//ak je plna
	{
		queueError(QERR_UP);
	}
	else
	{
		q->arr[q->b_index]= c;		//vlozenie znaku do pola na index b_index
		q->b_index=nextIndex(q->b_index);//posun b_index
	}
	//solved = FALSE;                  /* V pøípadì øe¹ení, sma¾te tento øádek! */
}
