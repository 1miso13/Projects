Popis:
Vytvořte jednoduchý grafický editor v Motifu, který umožňuje:
...
Jako základ projektu použijte připravený základ pro C nebo C++, který najdete zde. Editor může být tvořen např. widgety XmRowColumn, XmDrawingArea, XmLabel, XmPushButton. Výběr kresleného objektu a dalších parametrů by mělo být možné provádět stiskem tlačítek nebo z menu s indikací právě zvolené volby. Prakticky všechno můžete nalézt v příkladech. Ukázka ukončovacího dialogu je například v 03_quit/quit.c.

Hodnoceno bude:

    Splnění jednotlivých bodů zadání.
    Schopnost vytvořené aplikace správně reagovat na události Expose a Resize.
    Styl programování (přehlednost, struktura, ...). 
