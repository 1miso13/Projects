...
int otsu(unsigned long *hist, int n) {
   ...
   unsigned long   wB = 0,  wF = 0, threshold = 0;
   float mB, mF;  
   int   t;
...
int main(void)
{
   ...
   term_send_crlf();
	...
	while(f==0)
	{
		f= (int)fpga_read(FPGA_FRAME_CNT);
	}
	
   while (1) {
		
		f= (int)fpga_read(FPGA_FRAME_CNT);
		if(f%10==0 && f!=fOld) //každý 10. frame
		{
			for(i=0; i<PIXELS; i++){
				histogram[i] = fpga_read(FPGA_HISTOGRAM+i);//citanie histogramu
				fpga_write(FPGA_HISTOGRAM + i, 0);//vynulovanie
			}
			...
			if ((f % 100) == 0){
               print_results(f, threshold, histogram, PIXELS);
			}
			fOld = f;
		}
